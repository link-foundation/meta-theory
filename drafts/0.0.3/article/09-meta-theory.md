...
> $\displaystyle Sequence := LinkTree$

Последовательности реализуются как бинарные деревья связей-дуплетов: листья — элементы последовательности, узлы — связи $(левое, правое)$, соединяющие поддеревья. Например: $[1, 2, 3, 4] = ((1, 2), (3, 4))$.

**Уровень 2 (множества через последовательности):**

> $\displaystyle LinkSet := LinkTree\ |\ IsOrderedUniqueSequence(листья)$

Множества — это деревья связей с инвариантом: листья образуют строго возрастающий список без дубликатов.
...
Мета-ссылка — это лист дерева $LinkSet$, мета-дуплет — узел дерева (пара мета-ссылок), мета-ассоциативная сеть — функция из мета-ссылок в мета-дуплеты.
...
(* Мета-пространство ссылок: дерево связей *)
