...
async function extractWebPageContent() {
  ...
  content.headings = await page.$$eval('h1, h2, h3, h4', elements => {
  ...
  content.paragraphs = await page.$$eval('.tm-article-snippet__lead p, .tm-article-body p', elements =>
    elements.map(el => el.innerText.trim()).filter(p => p.length > 10)
  );

  // Extract code blocks
  content.codeBlocks = await page.$$eval('pre code, .tm-article-body pre', elements =>
    elements.map(el => el.innerText.trim()).filter(c => c.length > 0)
  );

  // Extract formulas (math content)
  content.formulas = await page.$$eval('.math, [class*="formula"], .katex, mjx-container', elements =>
    elements.map(el => el.innerText.trim()).filter(f => f.length > 0)
  ).catch(() => []);

  // Extract list items
  content.listItems = await page.$$eval('.tm-article-body li', elements =>
    elements.map(el => el.innerText.trim()).filter(li => li.length > 0)
  );

  // Extract important links (excluding navigation)
  content.links = await page.$$eval('.tm-article-body a[href]', elements =>
    elements.map(el => ({
      text: el.innerText.trim(),
      href: el.href
